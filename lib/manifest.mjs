...
async function listFiles(root, current = root) {
  ...
  for (const entry of await readdir(current, { withFileTypes: true })) {
    ...
    } else if (entry.isFile() && entry.name !== INTERNAL_MANIFEST && !entry.name.endsWith(".pyc")) {
...
export async function createSkillManifest({ skillRoot, packageVersion }) {
  ...
  return {
    schemaVersion: 1,
    packageVersion,
    skillName: "creating-ai-principle-videos",
...
export async function verifySkillManifest({ skillRoot, manifestPath }) {
  ...
  if (manifest.skillName !== "creating-ai-principle-videos") errors.push("manifest skillName is invalid");
