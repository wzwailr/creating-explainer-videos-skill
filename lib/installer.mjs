...
export const SKILL_NAME = "creating-ai-principle-videos";
...
function backupRoot(options, skillsRoot) {
  if (typeof options !== "string" && options.destination) {
    return path.join(path.dirname(skillsRoot), ".ai-principle-video-skill-backups");
  }
...
async function validateSkillRoot(skillRoot) {
  ...
  if (!/^---\s*[\s\S]*?\bname:\s*creating-ai-principle-videos\s*[\s\S]*?---/m.test(skillText)) {
    errors.push("SKILL.md frontmatter does not declare creating-ai-principle-videos");
  }
  for (const directory of ["agents", "scripts", "references", "assets"]) {
...
export async function rollbackSkill(options = {}) {
  ...
  if (options.backupPath) {
    ...
    if (path.dirname(selected) !== path.resolve(backupsRoot) || !path.basename(selected).startsWith(`${SKILL_NAME}-`)) {
      throw new Error("backup must be an exact creating-ai-principle-videos backup in this target's backup directory");
    }
  } else {
    const candidates = [];
