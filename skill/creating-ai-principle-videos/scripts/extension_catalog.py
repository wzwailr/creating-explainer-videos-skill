#!/usr/bin/env python3
"""Discover and validate declarative video-production extensions."""
...
def validate_extension(extension_root: Path) -> dict[str, Any]:
    ...
    profile_path = resolved_entrypoints.get("profile")
    if isinstance(profile_path, Path) and profile_path.is_file():
        try:
            profile = json.loads(profile_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            errors.append(f"profile is invalid JSON: {exc}")
