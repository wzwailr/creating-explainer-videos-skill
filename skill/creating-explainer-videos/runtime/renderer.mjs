...
function visualSystemCss() {
  return `.visual-program{position:absolute;inset:0;overflow:visible}.template-visual-stage{transform-style:preserve-3d;overflow:visible}.visual-motion-shell{position:absolute;transform-origin:center;transform-style:preserve-3d;will-change:transform,opacity}.visual-element{position:absolute;transform-origin:center;will-change:transform,opacity,filter}.visual-contained{inset:0;width:100%;height:100%}.visual-node,.visual-text,.visual-annotation,.visual-group{display:flex;align-items:center;justify-content:center;padding:18px 24px;text-align:center}.visual-node{border:3px solid var(--semantic,#d8fbff);border-radius:22px;font:750 36px/1.2 "Noto Sans SC","Microsoft YaHei",sans-serif}.visual-text{color:var(--semantic,#eefcff);font:800 40px/1.2 "Noto Sans SC","Microsoft YaHei",sans-serif}.visual-annotation{border-left:5px solid var(--semantic,#3af2ff);color:var(--semantic,#3af2ff);font:650 28px/1.3 "Noto Sans SC","Microsoft YaHei",sans-serif}.visual-group{border:2px solid var(--semantic,#d8fbff);border-radius:30px}.visual-shape{border:3px solid var(--semantic,#d8fbff)}.shape-circle{border-radius:50%}.shape-diamond{transform:rotate(45deg)}.visual-connector{inset:0;width:100%;height:100%;overflow:visible;pointer-events:none;color:var(--semantic,#3af2ff)}.visual-connector path[data-visual-path]{fill:none;stroke:currentColor;stroke-width:5;vector-effect:non-scaling-stroke;stroke-linecap:round;stroke-linejoin:round}.visual-connector marker path{fill:currentColor;stroke:none}.visual-connector [data-signal-dot]{fill:currentColor;filter:drop-shadow(0 0 8px currentColor)}.tone-info{--semantic:#3af2ff}.tone-process{--semantic:#9d83ff}.tone-success{--semantic:#70ef9b}.tone-warning{--semantic:#ffd75c}.tone-danger{--semantic:#ff5b78}.tone-accent{--semantic:#ff8fd8}.tone-muted{--semantic:#81929e}.spatial-chamber .visual-node{color:#f2fdff;background:linear-gradient(145deg,#0b2b38ed,#07141fdf);box-shadow:0 0 30px color-mix(in srgb,var(--semantic) 30%,transparent)}.spatial-chamber .visual-group{background:#07132199;box-shadow:inset 0 0 35px #06101bd9}.paper-theatre .visual-node{color:#171510;background:#fff9df;box-shadow:9px 11px 0 #12110e}.paper-theatre .visual-node.tone-success{background:#dff5c8}.paper-theatre .visual-node.tone-warning{background:#ffe493}.paper-theatre .visual-node.tone-danger{background:#f6b4af}.paper-theatre .visual-node.tone-process{background:#cbd5ff}.paper-theatre .visual-text{color:#171510}.ink-explainer .visual-node{color:#1c211e;background:#f8f2df;border-style:dashed}.ink-explainer .visual-text{color:#202225}.cover-topic-visual{position:absolute;inset:26px}.cover-topic-visual .visual-program{inset:0}.cover-topic-visual .visual-node{font-size:27px;padding:12px 16px}.cover-topic-visual .visual-text{font-size:30px}.cover-topic-visual .visual-annotation{font-size:22px}.cover-topic-visual .visual-connector path[data-visual-path]{stroke-width:4}`;
}
...
export async function buildCover(projectRoot) {
  ...
  const visualProgram = await loadVisualProgram(root);
  const visualCompilation = visualProgram?.complete === true ? await compileVisualProgram(root) : null;
  const coverSceneId = visualCompilation?.scenes?.[0]?.id;
  const topicVisual = coverSceneId ? visualCompilation.coverMarkupByScene[coverSceneId] : null;
  const coverVisual = topicVisual
    ? `<div class="cover-topic-visual" data-cover-source="visual-program">${topicVisual}</div>`
    : '<div class="cover-flow" data-cover-source="scaffold"><b>输入</b><i></i><b>内部变化</b><i></i><b>输出</b></div>';
  const html = `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><title>${escapeHtml(project.title)} 封面</title><link rel="stylesheet" href="./template/cover.css"><style>*{box-sizing:border-box}html,body{margin:0;width:1920px;height:1080px;overflow:hidden}body{font-family:"Noto Sans SC","Microsoft YaHei",sans-serif}.series-cluster{font-size:27px}.cover-question{font-weight:750}.cover-visual{display:grid;place-items:center;overflow:hidden}.cover-flow{display:grid;gap:34px;font-size:38px;text-align:center}.cover-flow i{height:5px;background:currentColor}${visualSystemCss()}</style></head><body><main class="cover ${escapeHtml(project.template)}"><div class="series-cluster"><span>EXPLAINER VIDEO</span><span>MECHANISM / PROCESS</span></div><h1 class="cover-title">${escapeHtml(project.title)}</h1><p class="cover-question">${escapeHtml(project.topic)}</p><section class="cover-visual" aria-label="${escapeHtml(project.topic)}">${coverVisual}</section></main></body></html>\n`;
